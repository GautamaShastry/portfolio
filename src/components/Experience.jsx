                ...
                {EXPERIENCES.map((experience, index) => (
                    ...
                    >
                        ...
                        <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:text-left md:pl-12' : 'md:text-right md:pr-12'}`}>
                            <span className="text-sm text-purple-500 font-medium">{experience.period}</span>
                        </div>
                        <motion.div whileHover={{ scale: 1.02 }} className={`md:w-1/2 ${index % 2 === 0 ? 'md:pr-12' : 'md:pl-12'} pl-8 md:pl-0`}>
                            <div className="p-6 bg-white dark:bg-neutral-800/50 rounded-2xl border border-gray-100 dark:border-neutral-700 shadow-lg hover:shadow-xl transition-shadow">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className='text-sm text-purple-500'>{experience.company}</p>
                                    </div>
                                </div>
                                <p className='text-gray-600 dark:text-gray-400 text-sm leading-relaxed mb-4'>{experience.description}</p>
                                <div className='flex flex-wrap gap-2'>
                                    {experience.technologies.map((tech, idx) => (
                                        <span key={idx} className='px-3 py-1 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-lg text-xs font-medium'>
                                            {tech}
                                        </span>
                                    ))}
                                </div>
